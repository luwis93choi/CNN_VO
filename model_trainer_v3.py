...
from dataloader_v3 import KITTI_Dataset_gray_full_sequence
...
class trainer_v3():
    ...
    def __init__(self, NN_model=None, checkpoint=None,
        ...
        self.train_sequence = data_sequence
        self.train_batch = train_batch
        
        self.valid_epoch = train_epoch
        self.valid_sequence = data_sequence
        ...
        Train_KITTI_Dataset = KITTI_Dataset_gray_full_sequence(name='KITTI_Train',
                                                                ...
                                                                sequence=data_sequence, verbose=0, mode='training')

        self.data_loader = torch.utils.data.DataLoader(Train_KITTI_Dataset, batch_size=self.train_batch, num_workers=8, shuffle=True, drop_last=True)
        ...
        # self.train_translation_loss = nn.L1Loss()
        ...
        # self.valid_translation_loss = nn.L1Loss()
    ...
    def train(self):
        ...
        for epoch in range(self.train_epoch):
            ...
            self.NN_model.train()
            self.data_loader.dataset.mode = 'training'
            for batch_idx, (prev_current_img, prev_current_odom) in enumerate(self.data_loader):
                ...
                self.train_loss_x = self.train_translation_loss_x(estimated_pose_vect.float()[:, 0], prev_current_odom.float()[:, 0])
                self.train_loss_y = self.train_translation_loss_y(estimated_pose_vect.float()[:, 1], prev_current_odom.float()[:, 1])
                self.train_loss_z = self.train_translation_loss_z(estimated_pose_vect.float()[:, 2], prev_current_odom.float()[:, 2])
                ...
                updates = []
                updates.append('\n')
                updates.append('[Train Epoch {}/{}][Progress : {:.2%}][Batch Idx : {}] \n'.format(epoch, self.train_epoch, batch_idx/len(self.data_loader), batch_idx))
            ...
            with torch.no_grad():
                ...
                self.NN_model.eval()
                self.data_loader.dataset.mode = 'validation'
                for batch_idx, (prev_current_img, prev_current_odom) in enumerate(self.data_loader):
                    ...
                    self.valid_loss_x = self.valid_translation_loss_x(estimated_pose_vect.float()[:, 0], prev_current_odom.float()[:, 0])
                    self.valid_loss_y = self.valid_translation_loss_y(estimated_pose_vect.float()[:, 1], prev_current_odom.float()[:, 1])
                    self.valid_loss_z = self.valid_translation_loss_z(estimated_pose_vect.float()[:, 2], prev_current_odom.float()[:, 2])
